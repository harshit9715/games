...
export default function Minesweeper() {
  ...
  const countNeighborMines = (grid: CellState[][], row: number, col: number) => {
    let count = 0
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[95vh] bg-black">
      ...
      <div className="grid gap-1 p-2 bg-gray-500 rounded-md shadow-lg">
        {grid.map((row, rowIndex) => (
          <div key={rowIndex} className="flex">
            {row.map((cell, colIndex) => (
              <button
                key={`${rowIndex}-${colIndex}`}
                className={`w-8 h-8 flex text-black items-center ml-1 first:ml-0 justify-center font-bold ${getCellColor(cell)} hover:bg-gray-400 focus:outline-none`}
