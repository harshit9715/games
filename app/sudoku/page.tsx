...
export default function SudokuGame() {
  ...
  const checkCompletedSections = (newBoard: number[][], row: number, col: number) => {
    const newCompletedSections = new Set(completedSections)
    const newAnimatingSections = new Set(animatingSections)
    
    // Check row
    if ([...Array(9)].every((_, i) => newBoard[row][i] !== 0)) {
      const rowCorrect = [...Array(9)].every((_, i) => newBoard[row][i] === game.solution[row][i])
    ...
    if ([...Array(9)].every((_, i) => newBoard[i][col] !== 0)) {
      const colCorrect = [...Array(9)].every((_, i) => newBoard[i][col] === game.solution[i][col])
    ...
    if ([...Array(3)].every((_, i) => 
        [...Array(3)].every((_, j) => 
          newBoard[squareRow * 3 + i][squareCol * 3 + j] !== 0
        )
      )) {
      const squareCorrect = [...Array(3)].every((_, i) =>
        [...Array(3)].every((_, j) =>
          newBoard[squareRow * 3 + i][squareCol * 3 + j] === game.solution[squareRow * 3 + i][squareCol * 3 + j]
        )
      )
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[95vh] bg-black">
          ...
          row.map((cell, j) => (
            <div
              ...
              className={`w-10 h-10 flex items-center justify-center text-black ${
                cell === 0 ? 'bg-white' : 'bg-gray-100'
              } ${
                i % 3 === 2 && i !== 8 ? 'border-b border-gray-400' : ''
              } ${j % 3 === 2 && j !== 8 ? 'border-r border-gray-400' : ''} ${
                ...
                animatingSections.has(`square-${Math.floor(i/3)}-${Math.floor(j/3)}`))
      ...
      <div className="mb-4 flex space-x-2">
        <Input
          ...
          min="1"
          max="9"
          value={selectedCell ? board[selectedCell[0]][selectedCell[1]] || '' : ''}
          onChange={handleInputChange}
          className="w-20 text-center"
          ref={inputRef}
          disabled={gameOver || isComplete}
        />
        <Button onClick={handleClearCell} disabled={gameOver || isComplete}>Clear</Button>
      </div>
